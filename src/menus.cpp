...
{
  int8_t  itrim = g_model.trimData[iLog].itrim;
  int16_t ret   = trimExpMu(abs(itrim),g_model.trimData[iLog].tmode);
    ...
    for(uint8_t j=0; j<2;j++){
      ...
      switch(j)
      {
        case 0:
          ...
          if(edit) {
            CHECK_INCDEC_H_MODELVAR_BF( event, g_model.trimData[iLog].tmode,    0,3);
          }
          break;

        case 1:
	  //uint8_t attr = sub==iLog ? INVERS : 0; 
	  lcd_outdezAtt( 9*FW, y, trimVal(iLog)*2, attr|PREC1 );
	  if(edit && outHelp[iLog] && BLINK_ON_PHASE) lcd_outdezAtt(14*FW, y, outHelp[iLog]   , outHelp[iLog]   ? BLINK|SIGN : 0);
	  else           lcd_outdezAtt(14*FW, y, g_model.limitData[iLog].offset , 0);
	  
	  if(edit && outHelp[iLog+4] && BLINK_ON_PHASE)lcd_outdezAtt(19*FW, y, outHelp[iLog+4] , outHelp[iLog+4] ? BLINK|SIGN : 0);
	  else           lcd_outdezAtt(19*FW, y, g_model.limitData[iLog+4].offset , 0);
	  if(edit && event== EVT_KEY_BREAK(KEY_RIGHT)) {
	    for(uint8_t i=0; i<NUM_CHNOUT;i++){
	      g_model.limitData[i].offset += outHelp[i];
	    }
	    g_model.trimData[sub].itrim     = 0;
...
void editSwitchVals(uint8_t event,uint8_t which,uint8_t mEdit,uint8_t x, uint8_t y, uint8_t idt)
{
  bool     edit    = mEdit==1;
  uint8_t  editAtt = mEdit ? (edit ? BLINK : BLINKF ) : 0;
...
void menuProcSwitchesCom(uint8_t event,bool nest);
void menuProcSwitchesNested(uint8_t event)
{
  menuProcSwitchesCom(event,true);
}
void menuProcSwitches(uint8_t event)
{
  menuProcSwitchesCom(event,false);
}
void menuProcSwitchesCom(uint8_t event,bool nested)
{
  static MState2 mstate2;
...
void editMixVals(uint8_t event,uint8_t which,uint8_t mEdit,uint8_t x, uint8_t y, uint8_t idt)
{
  bool     edit    = mEdit==1;
  uint8_t  editAtt = mEdit ? (edit ? BLINK : BLINKF ) : 0;
      ...
      )
  {
    ...
    if(line->showDat){ //show data 
      ...
      for(uint8_t j=0; j<6; j++){
        ...
        editMixVals(event,pgm_read_byte(&idx[j]),
                    subHor==j ? sel : 0,
                    pgm_read_byte(&colPos[j]),   y,line->idt); //2op
        
      }
//       if(!line->showCh)
//         editMixVals(event,2, subHor==4 ? sel : 0,  3*FW,   y,line->idt); //2op
//       editMixVals(event,0, subHor==5 ? sel : 0,  7*FW-2, y,line->idt); //3src
//       editMixVals(event,1, subHor==0 ? sel : 0, 11*FW-3, y,line->idt); //4prc
//       editMixVals(event,3, subHor==1 ? sel : 0, 14*FW-4,   y,line->idt); //3crv
//       editMixVals(event,4, subHor==2 ? sel : 0, 18*FW-4,   y,line->idt); //4sw
//       editMixVals(event,15,subHor==3 ? sel : 0, 20*FW-3,   y,line->idt); //2sw
...
void editExpoVals(uint8_t event,uint8_t which,uint8_t mEdit,uint8_t x, uint8_t y, uint8_t idt)
{
  bool     edit    = mEdit==1;
  uint8_t  editAtt = mEdit ? (edit ? BLINK : BLINKF ) : 0;
...
void menuProcExpoOne(uint8_t event)
{
  ...
  for(uint8_t i=0;i<6;i++){
    ...
    editExpoVals(event,i,sub==i,9*FW, y,FL_INST.currIDT());
    y+=FH;
  }
...
//       editExpoVals(event,0,subHor==0 ? sel : 0, 2*FW,   y,line->idt);
//       editExpoVals(event,1,subHor==1 ? sel : 0, 6*FW-3, y,line->idt);
//       editExpoVals(event,2,subHor==2 ? sel : 0, 9*FW-3, y,line->idt);
//       editExpoVals(event,3,subHor==3 ? sel : 0,13*FW,   y,line->idt);
//       editExpoVals(event,4,subHor==4 ? sel : 0,16*FW,   y,line->idt);
      if(lineEdit) lcd_barAtt( 4*FW,y,16*FW,BLINK);
    }
    if(FL_INST.isSelectedCh()) {
...
void menuProcModel(uint8_t event)
{
  ...
  for(uint8_t i=0; i<5; i++)
  {
    ...
    uint8_t attr = sub==i ? BLINK : 0; 
    if(i<3)
      lcd_putsmAtt(  0,    y,    PSTR("Name\t""Timer\t""Proto"),i,(subHor==0 && attr) ? INVERS : 0);
    else        
      lcd_putsm_P(  0,    y,    PSTR("Type\t""RM"),i-3);
    switch(i){
      case 0:    
        //lcd_putsAtt(    0,    2*FH, PSTR("Name"),sub==1 && subHor==0 ? BLINK:0);
        ...
        if(attr && subHor) {
          ...
          lcd_putcAtt((6+subHor-1)*FW, y, v,BLINK);
        }
        break;
      case 1:
        //lcd_putsAtt(    0,    4*FH, PSTR("Timer"),sub==3 && subHor==0 ? BLINK:0);
        putsTime(       5*FW, y, g_model.tmrVal,
                        ( subHor==1 ? attr:0),
                        ( subHor==2 ? attr:0) );
        lcd_putsmAtt(  12*FW, y, PSTR(" OFF\t ABS\t THR\tTHR%"),g_model.tmrMode,
                       ( subHor==3 ? attr:0));
        putsDrSwitches(16*FW, y, g_model.tmrSw , ( subHor==4 ? attr:0));

        if(attr) switch(subHor) 
        {
          case 1:
            {
          ...
          case 4:
            {
              ...
              CHECK_INCDEC_V_MODELVAR_BF( event,sw, MIN_DRSWITCH, MAX_DRSWITCH);
              g_model.tmrSw = sw;
            }
            break;
        }
	  ...
	  if(attr) switch(subHor)    {
	  case 1:
	    CHECK_INCDEC_V_MODELVAR_BF(event,g_model.protocol,0,PROT_MAX);
	    break;
	  case 2:      
	    CHECK_INCDEC_V_MODELVAR_BF(event,g_model.protocolPar,0,pgm_read_byte(p));
	    break;
	  }
        }
	break;
...
#define IST_FIX      4
  ...
  putsVBat(x-3+0*FW,1*FH, g_vbat100mV < g_eeGeneral.vBatWarn ? BLINK : 0);
  ...
  if(s_timerState != TMR_OFF){
    ...
    lcd_putsmAtt(   x+4*FW+1, FH*2, PSTR("\tTME\tTHR\tTHR%"),g_model.tmrMode,0);
...
#define TL 27
    ...
    uint8_t xm,ym;
    xm=x[i];
    int8_t val = max((int8_t)-(TL+1),min((int8_t)(TL+1),g_model.trimData[convertMode(i)].itrim));
    if(vert[i]){
      ym=31;
    ...
    switch(g_istTrimState){
      ...
      case IST_FIX:
        ...
        if(s_istTmr-- == 0){
          ...
          for(uint8_t i=0; i<4;i++){
            if(!trimAssym[i])
              g_model.trimData[i].itrim = trimRevertM(s_istAnas[i]-anas2[i],g_model.trimData[i].tmode);
          }
        }else{
